...
class ExpenseImportService
{
    ...
    private function process(UploadedFile $file, bool $save): array
    {
        ...
        foreach ($grid as $row) {
            ...
            $paymentMethodName = $cell('Payment Method');
            ...
            $uom = $cell('UOM');
            $receiverName = $cell('Receiver Name');
            $receiverMobile = $cell('Receiver Mobile');
            $companyName = $cell('Company Name');
            $description = $cell('Description');

            if ($dateRaw === '' && $branchName === '' && $accountName === '' && $particularCode === '' && $amountRaw === '' && $qtyRaw === '' && $rateRaw === '') {
                continue;
            }
